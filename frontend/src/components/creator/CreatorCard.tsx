  ...
  return (
    <Link href={`/${user.username}`} className="card block overflow-hidden hover:shadow-md transition-shadow">
      ...
      <div className="p-4 pt-0">
        <div className="relative z-10 -mt-8 mb-3">
          {user.avatar_url ? (
            ...
            <img
              src={user.avatar_url}
              alt={user.username}
              className="h-16 w-16 rounded-full border-4 border-white dark:border-gray-900 object-cover shadow"
            />
          ) : (
            <div className="flex h-16 w-16 items-center justify-center rounded-full border-4 border-white dark:border-gray-900 bg-brand-100 text-xl font-bold text-brand-600 shadow">
        ...
        {profile.category && (
          <span className="mt-2 inline-block rounded-full bg-gray-100 dark:bg-gray-800 px-2 py-0.5 text-xs text-gray-500 dark:text-gray-400">
            {profile.category}
          </span>
        )}

        {profile.description && (
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400 line-clamp-2">{profile.description}</p>
        )}
